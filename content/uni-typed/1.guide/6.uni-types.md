---
title: uni-types
description: 为 Vue v3 uni-app、uni-cloud 和 uni-ui 组件提供 TypeScript 类型
...
:pm-install{name="-D @uni-helper/uni-types"}
...
![示例1](/uni-typed/uni-app-types-example.jpg)
![示例2](/uni-typed/uni-cloud-types-example.jpg)
![示例3](/uni-typed/uni-ui-types-example.jpg)
...
function onScroll(text: string, event: ScrollViewOnScrollEvent) {
  // ...
}
</script>

<template>
  <scroll-view @scroll="onScroll('ScrollViewA', $event)" />
  <scroll-view @scroll="onScroll('ScrollViewB', $event)" />
</template>
```
