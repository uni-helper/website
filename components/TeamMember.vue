...
<template>
  ...
  <div grid="~ sm:cols-2 md:cols-3 lg:cols-4 gap-x-3 gap-y-20 items-center" p-10>
    <template v-for="member, _idx of members" :key="_idx">
      <div text-center>
        <img loading="lazy" width="100" height="100" m-auto h-25 min-h-25 min-w-25 w-25 rounded-full :src="member.avatar">
        ...
        <div flex="~ inline gap-2" py-2>
          <a
            ...
            :href="member.htmlUrl" target="_blank" rel="noopener noreferrer" aria-label="Anthony Fu on GitHub"
          />
          <a
            ...
            aria-label="Anthony Fu on Twitter"
          />
        </div>
      </div>
    </template>
